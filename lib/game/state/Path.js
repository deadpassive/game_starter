    ...
    GClass.extend(Object, $this, {
        ...
        draw: function(ctx, xOffset, yOffset, tileSize) {
            if (this._path && this._path.length > 0) {
                ...
                ctx.beginPath();
                var startPoint = this._entity.getLastPixelPos();
                ctx.moveTo(startPoint.x + tileSize/2, startPoint.y + tileSize/2);
                for (var i = 0; i < this._path.length; i++) {
                    var tile = this._path[i];
                    ctx.lineTo((tileSize * tile.x) + xOffset + tileSize/2, (tileSize * tile.y) + yOffset + tileSize/2);
                }
                ctx.strokeStyle = "blue";
                ctx.lineWidth = 3;
                ctx.stroke();
                                
                var targetXPix = (tileSize * this._goal.x) + xOffset;
                var targetYPix = (tileSize * this._goal.y) + yOffset;
